...
contract VyronXStakingV4Test is Test {
    ...
    function test_MLM_PaysOnNonElitePool() public {
        ...
        vm.startPrank(investor);
        staking.setReferrer(promoter);
        staking.stake(0, 1000e18); // Starter (not Elite)
        vm.stopPrank();

        // Fast forward 10 days (yield: $1000 * 0.0011 * 10 = $11 > $10 min)
        vm.warp(block.timestamp + 10 days);
        vm.prank(investor);
        staking.claimDailyEarnings(0);
    ...
    function test_MLM_NoEliteNoPay() public {
        ...
        usdt.approve(address(staking), type(uint256).max);
        staking.setReferrer(owner);
        staking.stake(0, 1100e18); // Starter only
        vm.stopPrank();
