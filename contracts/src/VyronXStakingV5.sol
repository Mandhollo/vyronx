...
contract VyronXStakingV5 is ReentrancyGuard {
    ...
    struct Pool {
        ...
        bool active;
        string tierName;
        uint256 minStakeUsdt;   // per-stake minimum (0 = uses global $50)
        uint256 maxStakeUsdt;   // per-stake maximum (0 = NO LIMIT)
    }
    event PoolLimitsChanged(uint256 indexed poolId, uint256 minStakeUsdt, uint256 maxStakeUsdt);
    ...
    event AffiliateLevelUpdated(uint256 indexed level, uint256 commissionBps, uint256 minStakeUsdt, uint256 minDirectReferrals);
    ...
    constructor(address _usdt, address _vyrToken, address _usdtCollector, address _rootReferrer) {
        ...
        // V5: per-pool stake limits (max = 0 means NO LIMIT)
        pools[0] = Pool({lockPeriodDays: 30, dailyRateBps: 11, active: false, tierName: "Starter", minStakeUsdt: 50e18, maxStakeUsdt: 100e18});
        pools[1] = Pool({lockPeriodDays: 60, dailyRateBps: 23, active: false, tierName: "Growth", minStakeUsdt: 50e18, maxStakeUsdt: 250e18});
        pools[2] = Pool({lockPeriodDays: 180, dailyRateBps: 33, active: false, tierName: "Pro", minStakeUsdt: 50e18, maxStakeUsdt: 500e18});
        ...
        affiliateLevels[10] = AffiliateLevel(700, 1100e18, 11);
    ...
    function stake(uint256 poolId, uint256 usdtAmount) external nonReentrant {
        ...
        // V5: per-pool limits
        uint256 minReq = pool.minStakeUsdt > 0 ? pool.minStakeUsdt : 50e18;
        require(usdtAmount >= minReq, "Below pool minimum");
    ...
    function claimDailyEarnings(uint256 stakeIndex) external nonReentrant {
        ...
        // V5: unilevel commissions are generated by yield claims from ANY pool
        // (Starter/Growth/Pro/Elite stakers all participate in the multi-level system).
        // The 360-day ACCELERATOR (10% instant USDT + auto-liquidation) stays Elite-only.
    ...
    function _payAffiliateCommissions(address staker, uint256 profitUsdt) internal {
        ...
        for (uint256 level = 0; level < 11; level++) {
            ...
            uint256 uplineDirects = _countQualifiedDirects(upline);
            uint256 uplineTotalStaked = _getTotalStaked(upline);

            if (uplineTotalStaked >= al.minStakeUsdt && uplineDirects >= al.minDirectReferrals && hasActivePool360(upline)) {
    ...
    /// @notice V5: SUM of all ACTIVE stakes (not withdrawn AND still locked).
    /// Dynamic: when a stake EXPIRES (lock ends) or is withdrawn, it stops counting
    /// immediately and the member falls back to the previous level automatically.
    function _getTotalStaked(address user) internal view returns (uint256) {
        Stake[] storage stakes = userStakes[user];
        uint256 total = 0;
        for (uint256 i = 0; i < stakes.length; i++) {
            if (!stakes[i].withdrawn && block.timestamp < stakes[i].lockEndTime) {
                total += stakes[i].usdtAmount;
            }
        }
        return total;
    }

    /// @notice V5: directs with at least $100 staked (sum) count as qualified
    uint256 public qualifiedDirectMin = 100e18;
    ...
    function _countQualifiedDirects(address user) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < directs.length; i++) {
            if (_getTotalStaked(directs[i]) >= qualifiedDirectMin) count++;
        }
        return count;
    }

    function getAffiliateQualification(address user) external view returns (uint256 totalStaked, uint256 qualifiedDirects) {
        return (_getTotalStaked(user), _countQualifiedDirects(user));
    }
    ...
    // ═══ Admin ═══
    /// @notice V5: Update affiliate level requirements (owner only, via admin panel)
    function setAffiliateLevel(uint256 level, uint256 commissionBps, uint256 minStakeUsdt, uint256 minDirectReferrals) external onlyOwner {
        require(level < 11, "Invalid level");
        require(commissionBps <= 1000, "Commission too high");
        affiliateLevels[level] = AffiliateLevel(commissionBps, minStakeUsdt, minDirectReferrals);
        emit AffiliateLevelUpdated(level, commissionBps, minStakeUsdt, minDirectReferrals);
    }
    ...
    /// @notice V5: minimum staked (sum) for a direct to count as qualified (default $100)
    function setQualifiedDirectMin(uint256 _min) external onlyOwner {
        qualifiedDirectMin = _min;
    }

    /// @notice V5: Set per-pool stake limits (owner, via admin panel). max = 0 → NO LIMIT
    function setPoolLimits(uint256 poolId, uint256 minStakeUsdt, uint256 maxStakeUsdt) external onlyOwner {
        require(poolId < POOL_COUNT, "Invalid pool");
        require(minStakeUsdt > 0, "Min must be > 0");
        pools[poolId].minStakeUsdt = minStakeUsdt;
        pools[poolId].maxStakeUsdt = maxStakeUsdt;
        emit PoolLimitsChanged(poolId, minStakeUsdt, maxStakeUsdt);
    }
