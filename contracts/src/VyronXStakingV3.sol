...
/// @title VyronX Staking V3
/// @notice Voucher = license: creates virtual Pool 360 stake + accelerator entry.
/// @notice Voucher does NOT yield or return principal, but fully participates in accelerator (100% → 12h grace).
contract VyronXStakingV3 is ReentrancyGuard {
    ...
    // ═══ Vouchers (V3: license WITH accelerator — needs usdtValue for accelerator % calc) ═══
    struct Voucher {
        address recipient;
        uint256 poolId;
        uint256 usdtValue;     // V3: value for accelerator % calculation
        uint256 expiry;
        bool redeemed;
        bool cancelled;
    }
    ...
    uint256 public constant ACCELERATOR_COMM_BPS = 1000;
    ...
    // ═══ Payout (V3: voucher pays nothing but gets withdrawn flag set) ═══
    ...
    // ═══ Vouchers (V3: license WITH accelerator participation) ═══
    ...
    function redeemVoucher(uint256 voucherId) external nonReentrant {
        ...
        // V3: Create accelerator entry — voucher holder can reach 100% + 12h grace
        _createAccelerator(msg.sender, userStakes[msg.sender].length - 1);
